...
const formSchema = z.object({
    setpoint: z.coerce
        ...
        .max(31, { message: "Setpoint must be less than 32" }),
    lowerLimit: z.coerce.number().min(5, {
        message: "Lower limit must be greater than 5",
    }),
    upperLimit: z.coerce.number().max(31, {
        message: "Upper limit must be less than 32",
    }),
    systemMode: z.enum(["cool", "heat", "ventilation", "auto_heat"]),
    fanStatus: z.enum(["low", "mid", "high", "auto_high"]),
})
    ...
    async function onSubmit(values: z.infer<typeof formSchema>) {
        startTransition(async () => {
            ...
            try {
                ...
                console.error(error)
                setError(new Error("Failed to apply settings"))
            }
        })
    }
